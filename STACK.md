...
5. **Audio** : `speechSynthesis` reste le mécanisme MVP, avec détection de voix fr — si aucune voix française : l'aide du barreau 3 reste purement visuelle (déjà prévue au cahier). Option post-MVP notée : pré-générer les fichiers audio des lettres (`say -v` macOS) et ne garder speechSynthesis qu'en repli. **Depuis #124, la DICTÉE a sa propre voix** : Piper (`fr_FR-siwis-medium`) embarqué dans l'image de production, servi par `/api/voix/mot` pour les seuls mots des listes du compte, mis en cache sur le disque du conteneur. **Depuis #126, toute voix de l'app passe par Piper** — le nom de la lettre du barreau 3 et les consignes lues (V2, V3 ; `/api/voix/consigne`, phrases de `core/consignes.ts`, seules acceptées). Le nom de la lettre (`/api/voix/lettre`, noms écrits en toutes lettres par `core/nomDeLettre.ts` : le moteur lit « à » comme « a » et reste muet sur la ponctuation). `speechSynthesis` reste le repli — silencieux — partout où Piper n'est pas : développement, CI, hors ligne, synthèse en échec.
