...
/* Régression #76. La rangée de points de l'entête montrait douze pastilles
   ...
   Un point vaut désormais un MOT de l'exercice servi (#107).

   Ces tests tiennent les deux bouts : N mots tapés allument exactement N
...
test.describe("les points de l'entête comptent les mots", () => {
  ...
  test('la rangée porte un libellé lisible, et compte l\'exercice et non douze', async ({ page }) => {
    ...
    await expect(page.getByText('Mots de cet exercice')).toBeVisible();

    /* Le nombre de points est celui de l'exercice réellement servi (8 à 12
  ...
  test('après N mots tapés, N points sont allumés', async ({ page }) => {
    await ouvrir(page, 'fr-FR', 1, false, 'Joueur 1', 'decouverte', 0, LONGUE);
    await page.getByRole('button', { name: 'On commence !' }).click();
    await expect(page.locator('body')).toHaveAttribute('data-vue', 'V4');
