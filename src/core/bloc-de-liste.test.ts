...
import { composerBlocDeListe } from './generator';
...
describe('composerBlocDeListe', () => {
  it('sert les mots de la famille tels quels, dédoublonnés', () => {
    const bloc = composerBlocDeListe(['dinosaure', 'dinosaure', 'papi'], 'fr-FR', 7);
  ...
  it("accepte tout ce que le clavier écrit, refuse le reste", () => {
    const bloc = composerBlocDeListe(['kiwi', 'vélo', 'château'], 'fr-FR', 7);
  ...
  it('borne le bloc à 12 items', () => {
    ...
    expect(composerBlocDeListe([...new Set(mots)], 'fr-FR', 7).length).toBeLessThanOrEqual(12);
  });
});
...
describe('mode libre : tout ce que le clavier sait écrire', () => {
  it("accepte une phrase avec apostrophe, chiffres et espaces", () => {
    const items = composerBlocDeListe(["le 20 octobre c'est papa"], 'fr-FR', 1);
    expect(items.map((i) => i.texte)).toEqual(["le 20 octobre c'est papa"]);
  });

  it('accepte accents directs et cédille', () => {
    const items = composerBlocDeListe(['où est le garçon', 'à côté'], 'fr-FR', 1);
  ...
  it('refuse les accents à touche morte (â, ê, ë) : deux frappes pour un caractère', () => {
    expect(composerBlocDeListe(['la fête'], 'fr-FR', 1)).toHaveLength(0);
  });

  it("refuse ce que la disposition ne peut pas écrire", () => {
    expect(composerBlocDeListe(['ЖЖЖ', 'papa'], 'fr-FR', 1).map((i) => i.texte)).toEqual(['papa']);
  });
});
